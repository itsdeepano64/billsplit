            ...
            return (
              <div key={t.id} className="bg-card rounded-2xl border border-border px-4 py-3 flex items-center gap-3">
                ...
                <div className={`flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center text-lg
                  ${isDeShea ? "bg-indigo-500/15" : "bg-emerald-500/15"}`}>
                  {isDeShea ? "💜" : "💚"}
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center">
          ...
          <div className="relative w-full max-w-lg bg-card rounded-t-3xl border-t border-x border-border p-5 pb-10 animate-fade-in">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {/* Notes */}
              <div className="space-y-1.5">
                <label className="text-xs text-muted-foreground uppercase tracking-wide">Notes / Reason</label>
                <input
                  value={notes} onChange={(e) => setNotes(e.target.value)}
                  placeholder="e.g. Splitting rent this month"
                  className={inputCls}
                />
              </div>
              ...
              <button
                type="submit"
                disabled={saving}
                className="w-full bg-primary text-primary-foreground rounded-xl py-4 font-semibold disabled:opacity-50 transition-all"
              >
